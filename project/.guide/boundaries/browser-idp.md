...
- Prevent browser-held credentials from becoming the default downstream identity transport when an entry is present.
- Ensure browser login uses an explicit PKCE-based interactive flow.
- Ensure successful browser login ends in entry-managed session state instead of browser-held downstream bearer-token use.
...
- Terminate browser authentication into a first-party entry-managed `cookie_session` after login completion.
- Do not treat browser bearer tokens as the default steady-state session mechanism when an entry is present.
